...
public class ApiV1BlogController {
    ...
    public RsData<BlogDto> create(
        ...
        Blog blog = blogService.write(userDetails.getUserId(), reqbody, thumbnailUrl);
    ...
    public RsData<BlogDto> modify(
        ...
        BlogDto blogdto = blogService.modify(userDetails.getUserId(), id, reqbody, thumbnailUrl);
    ...
    public RsData<BlogDto> saveDraft(
        ...
        Blog blog = blogService.saveDraft(userDetails.getUserId(), blogId, reqbody, thumbnailUrl);
        return new RsData<>("201-1", "블로그 임시저장이 완료되었습니다.", new BlogDto(blog));
    }

    @GetMapping("/drafts")
    @Operation(summary = "블로그 임시저장 글 다건 조회")
    public RsData<List<BlogDraftDto>> getDrafts(@AuthenticationPrincipal CustomUserDetails userDetails) {
        List<BlogDraftDto> draftDtos = blogService.findDraftsByUserId(userDetails.getUserId());
    ...
    public RsData<LikeResponse> increaseLike(@PathVariable Long id, @AuthenticationPrincipal CustomUserDetails userDetails) {
    ...
    public RsData<BookmarkResponse> addBookmark(@PathVariable Long id, @AuthenticationPrincipal CustomUserDetails userDetails) {
        boolean on = blogBookmarkService.bookmarkOn(userDetails.getUserId(), id);
